import { getDebugLogger } from './debugLog';
...
// console.log('case-sensitive diacritic normalization map:', caseSensitiveDiacriticNormalizationTable);
// console.log('case-insensitive diacritic normalization map:', caseInsensitiveDiacriticNormalizationTable);
// console.log('case-sensitive diacritic regexp map:', caseSensitiveDiacriticRegexpTable);
// console.log('case-insensitive diacritic regexp map:', caseInsensitiveDiacriticRegexpTable);
...
export function cutAroundMatch(
    ...
    const logger = getDebugLogger('cut-around', debug);
    ...
    if (!match) {
        ...
        logger.log(`No match for "${pattern}" in "${corpus}", so returning.`);
        return;
    }
    ...
    const startPos = Math.max(Math.min(match.startPos - Math.floor(buffer / 2), corpus.length - fragmentLength), 0);
    const endPos = Math.min(startPos + fragmentLength, corpus.length);

    // compile the result
    const result = (startPos ? '…' : '') + corpus.substring(startPos, endPos) + (endPos < corpus.length - 1 ? '…' : '');

    logger.log(
        `Corpus: ${corpus.length} chars;`,
        `pattern: ${pattern.length} chars;`,
        `match is at [${match.startPos}-${match.startPos + match.searchText.length}];`,
        `returning fragment from [${startPos}-${endPos}], length: ${result.length}.`,
        result
    );
