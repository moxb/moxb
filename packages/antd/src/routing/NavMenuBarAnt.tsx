...
export const NavMenuBarAnt = inject('locationManager')(
    observer(
        class /**
         * This widget show an Ant menu bar, based on the state-space.
         */
        NavMenuBarAnt<DataType> extends React.Component<NavMenuProps<DataType>> {
            ...
            _renderSubStateLink(
                ...
                const { label, key, menuKey, newWindow, itemClassName, linkClassName, linkStyle, noLink, title } =
                    state;
            ...
            _renderSubStateGroup(
                ...
                return (
                    <Menu.SubMenu
                        ...
                        style={linkStyle}
                    >
                        {subStates!.map((s) => this._renderSubStateElement(states, s))}
                    </Menu.SubMenu>
                );
            }

            _renderSeparator(state: SubStateInContext<UIFragment, any, any>) {
                return <Menu.Divider key={state.menuKey} />;
            }

            _renderSubStateElement(
                states: LocationDependentStateSpaceHandler<UIFragment, UIFragmentSpec, DataType>,
                state: SubStateInContext<UIFragment, UIFragmentSpec, DataType>
            ) {
                ...
                return separator
                    ...
                    : isGroupOnly
                    ? this._renderSubStateGroup(states, state)
                    : this._renderSubStateLink(states, state);
            }

            render() {
                const states = this._getLocationDependantStateSpaceHandler();
                ...
                return (
                    ...
                    >
                        {states
                            ...
                            .map((state) => this._renderSubStateElement(states, state))}
