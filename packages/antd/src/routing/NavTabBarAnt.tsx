...
export const NavTabBarAnt = inject('locationManager')(
    observer(
        class /**
         * This widget show an Ant tab bar, based on the state-space.
         */
        NavTabBarAnt<DataType> extends React.Component<NavTabProps<DataType>> {
            _getLocationDependantStateSpaceHandler() {
                        ...
                        {states.isSubStateActive(state) &&
                            renderSubStateCore<DataType>({
                                ...
                                navControl: {
                                    ...
                                    getAncestorNames: () => [
                                        ...(navControl ? navControl.getAncestorNames() : []),
                                        parentName,
                                    ],
                                    ...
                                    isActive: () =>
                                        (!navControl || navControl.isActive()) && states.isSubStateActive(state),
                                },
                            })}
                    </TabPane>
                );
                ...
                return (
                    ...
                    >
                        {states
                            ...
                            .map((state) => this._renderStateTabPane(id, states, state))}
                        {extras.map((f) => renderUIFragment(f))}
                        {!activeKey && this._renderErrorPanel(id, stateSpace.fallback, part)}
                    </Tabs>
                );
            }
        }
