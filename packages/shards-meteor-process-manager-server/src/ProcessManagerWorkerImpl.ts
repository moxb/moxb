...
import type { ProcessManagerWorkerProps } from './worker';
import { ProcessCollection } from '@moxb/shards-meteor-process-manager-core';

interface ProcessTracker<ScopeData> extends ProcessDefinition<ScopeData> {
    shouldRun: boolean;
}

export class ProcessManagerWorkerImpl<ScopeData = void> {
    private readonly _instanceId: string;

    constructor(private readonly props: ProcessManagerWorkerProps<ScopeData>) {
    ...
    private readonly _processes: Record<string, Record<string, ProcessTracker<ScopeData>>> = {};

    private _defineImportProcess(scopeId: string, definition: ProcessDefinition<ScopeData>) {
    ...
    private _defineProcesses(scopeId: string, params: DefineProcessesParams = {}) {
        ...
        const scopeData = this.props.getScopeData(scopeId);
        const processes = this.props.getProcessList(scopeData);
    ...
    private _launchProcess(scopeId: string, processId: string): ProcessEndState {
        ...
        try {
            ...
            const coreContext: ProcessContext<ScopeData> = {
                scopeId,
                reportProgress,
            ...
            const { getScopeData } = this.props;
            const scopeData = getScopeData(scopeId);

            const context: ProcessContext<ScopeData> & ScopeData = {
                ...coreContext,
                ...scopeData,
            };
