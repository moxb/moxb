...
interface ProcessManagerProps<ExtraContext = {}> {
    /**
...
class ProcessManagerImpl<ExtraContext = {}> {
    ...
    private _launchProcess(scopeId: string, processId: string): ProcessEndState {
        ...
        try {
            ...
            const coreContext: ProcessContext<ExtraContext> = {
                ...
                runProcess: (process, params = {}) => {
                    const { surviveFailure, surviveStopping } = params;
                    const result = this._launchProcess(scopeId, process.processId);
                    switch (result) {
                        case 'done':
                            return result;
                        case 'stopped':
                        ...
                        case 'failed':
                            if (surviveFailure) {
                                ...
                                throw new Error(`Failed to ${process.name}.`);
                            }
                    }
                },
            };
